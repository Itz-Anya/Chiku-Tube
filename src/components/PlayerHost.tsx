...
export function PlayerHost() {
  ...
  // Track which video ids we've already tried to auto-recover so we only
  // refetch once per id before giving up and marking it broken.
  ...
  useEffect(() => {
    ...
    // If we already have URLs from initial load, use them. Otherwise refetch.
    const useDirect = (v.videoUrl || v.audioUrl) && player.refetchKey === 0;
    if (useDirect) {
    ...
    >
      ...
      >
        <video
          ...
          onError={() => {
            setLoading(false);
            // First failure for this id → try one silent refetch (the stream
            // URL likely expired). On the second failure, mark it broken so
            // every shelf hides the card.
            if (v?.id && !expiryHandledRef.current.has(v.id)) {
              expiryHandledRef.current.add(v.id);
              refetchStream();
              return;
            }
            setStreamError(true);
            if (v?.id) markBroken(v.id);
