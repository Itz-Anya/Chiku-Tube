...
export const generateBecauseYouWatched = createServerFn({ method: "POST" })
  .inputValidator(
    (input: { recentTitles: string[]; likedTitles?: string[] }) =>
      z
        .object({
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const keywords = await callAIForKeywords(
        "You generate YouTube search phrases that match a user's taste. CRITICAL: ALWAYS match the language and culture of the input titles. If titles are in Kannada/Hindi/Tamil/Telugu/Spanish/Korean/Japanese/etc., return phrases IN THAT SAME LANGUAGE/SCRIPT and culture. Never default to English. Liked videos signal stronger preference than watched. Return diverse, specific phrases (2–5 words each) — mix close picks and tangential discoveries within the same genre/language.",
        `LIKED (high signal):\n${liked.length === 0 ? "(none)" : liked.map((t, i) => `${i + 1}. ${t}`).join("\n")}\n\nWATCHED (medium signal):\n${data.recentTitles.map((t, i) => `${i + 1}. ${t}`).join("\n")}\n\nReturn 6 YouTube search phrases in the SAME language as the titles above.`,
      );
      return {
  ...
  .handler(async ({ data }) => {
    try {
      ...
      const keywords = await callAIForKeywords(
        ...
        `Hint about user taste: ${data.hint || "general audience"}\nSeed: ${seed}\n\nReturn 5 trending YouTube search phrases.`,
      );
